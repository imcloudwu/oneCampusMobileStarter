...
class KeyinByCodeViewCtrl: UIViewController,UIAlertViewDelegate,UITextFieldDelegate,UIWebViewDelegate {
    ...
    func JoinAsParent(){
        ...
        if let code = self.code.text, let relationship = self.relationship.text{
            ...
            let rsp = Resource.Connection.SendRequest(self._DsnsItem.AccessPoint, contract: "auth.guest", service: "Join.AsParent", body: "<Request><ParentCode>\(code)</ParentCode><Relationship>\(relationship)</Relationship></Request>")
